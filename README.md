...
`conda create --name VASIL --file <the spec-file of your OS>` ([see conda doc](https://conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html#activating-an-environment), but some package might still be missing, e.g. those we had to installed via `pip`)
