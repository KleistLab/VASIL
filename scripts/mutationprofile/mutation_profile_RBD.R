...
## Grep for only spike ("S:") and RBD/NTD mutations.
mutation_lists <- c()
for (i in 1:number_lineages){
  ...
  m1 <- sort(m1)
  m1_sites <- parse_number(m1)
  m1_rbd <- m1[unlist(lapply(m1_sites, function(x) x %in% c(NTD_positions,RBD_positions)))]
  if (length(mutation_lists) == 0){
    mutation_lists <- cbind(lineages_l[i], paste(m1_rbd, collapse = "/"))
  }else{
    mutation_lists <- rbind(mutation_lists, 
                            cbind(lineages_l[i], paste(m1_rbd, collapse = "/")))
  }
}
colnames(mutation_lists) <- c("lineage", "mutated_sites_RBD")
mutation_lists <- as.data.frame(mutation_lists)
rm(m1); rm(m1_rbd);rm(m1_sites)
...
mydf <- data.frame(row.names = colnames(MP2), 
                   category = c(rep("NTD", length(sites_ntd_x)), 
