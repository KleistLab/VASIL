...
outputfile_mutationprofile_mutations_spike <- paste0(outputdir,"/",output_prefix_for_file,"_spike_mutations.csv")
...
RBD_NTD_positions <- c(NTD_positions, RBD_positions)
...
lineages_of_interest <-c("BA.2", "BA.2.12.1", "BA.4", "BA.5","B.1.1.7") #lineages_pango 
...
# rewrite deletions:
x <- grep("del",mutations$mutation)
mutations$mutation[x] <- gsub("/",":",mutations$mutation[x])


# filter for Spike:
mutations_s <- subset(mutations, gene=="S")
# filter for rbd and NTD:
mutations_s_rbd_ntd <- subset(mutations_s, codon_num%in% RBD_NTD_positions)
#write.csv(mutations_s_rbd, "mutationprofile_outbreakinfo.csv", quote = FALSE, row.names = FALSE)


mutation_lists_spike <- c()
mutation_lists_rbdntd <- c()
for (lin in lineages_of_interest){
  lin_mut_s <- subset(mutations_s, lineage==lin)
  lin_mut_s_muts <- paste0(lin_mut_s$mutation, collapse = "/")
  lin_mut_s_muts <- toupper(gsub("s:","",lin_mut_s_muts))
  lin_mut_rbdntd <- subset(mutations_s_rbd_ntd, lineage==lin)
  lin_mut_rbdntd_muts <- paste0(lin_mut_rbdntd$mutation, collapse = "/")
  lin_mut_rbdntd_muts <- toupper(gsub("s:","",lin_mut_rbdntd_muts))
  
  if (length(mutation_lists_spike) == 0){
    mutation_lists_spike <- cbind(lin, lin_mut_s_muts)
    mutation_lists_rbdntd <- cbind(lin, lin_mut_rbdntd_muts)
  }else{
    mutation_lists_spike <- rbind(mutation_lists_spike, cbind(lin, lin_mut_s_muts))
    mutation_lists_rbdntd <- rbind(mutation_lists_rbdntd, cbind(lin, lin_mut_rbdntd_muts))
  }
}
colnames(mutation_lists_spike) <- c("lineage", "mutated_sites_spike")
mutation_lists_spike <- as.data.frame(mutation_lists_spike)
colnames(mutation_lists_rbdntd) <- c("lineage", "mutated_sites_rbd_ntd")
mutation_lists_rbdntd <- as.data.frame(mutation_lists_rbdntd)

  
write.csv(mutation_lists_rbdntd, file=outputfile_mutationprofile_mutations, row.names = FALSE, quote = FALSE)
write.csv(mutation_lists_spike, file=outputfile_mutationprofile_mutations_spike, row.names = FALSE, quote = FALSE)
